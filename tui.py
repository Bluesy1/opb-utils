...
def start_tui():
    ...
    try:
        ...
        if not TESTING:
            ...
            shutil.copytree(src=full_path.parent, dst=WRITE_PATH / full_path.parent.name)
            CWD = WRITE_PATH / full_path.parent.name
            pr_body = [f"-b 'Closes #{issue}'" for issue in issues]
            pr_body.append("-b 'OPB 000: https://ca.prairielearn.com/pl/course_instance/4024/instructor/course_admin/questions'")
            commands = [
                f"git checkout -b --no-track {branch_name!r} origin/main",
                "git add .",
                "git commit -m 'Autogenerated Template'",
                f"git push -u origin {branch_name!r}",
                f"gh pr create --draft -t {branch_name!r} {' '.join(pr_body)} --assignee {GITHUB_USERNAME!r}",
            ]
